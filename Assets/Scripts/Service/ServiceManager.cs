...
public class ServiceManager : MonoBehaviour
{
    ...
    public float minDelayAfterReward = 30f;
    ...
    //public RemoteConfigLoader remoteConfigLoader;
    ...
    public static void OnReward(bool isSuccess)
    {
        ...
        interReadyTime = Mathf.Max(interReadyTime, Time.unscaledTime + Instance.minDelayAfterReward);
    }
    ...
    {
        //if (Instance.IsStaging || !IAPProductDefine.IsAdAllowed)
        ...
        GlobalData.Instance.SetTimeoutUnscaledTime(() =>
        {
    ...
    public static void AddTimeCounter(float time)
    {
        var newTime = Time.unscaledTime + time;
        interReadyTime = Mathf.Max(interReadyTime, newTime);
    }

    public static bool IsTimerInterReady => interReadyTime < Time.unscaledTime;
    public static bool IsTimerReady(float offset) => interReadyTime + offset < Time.unscaledTime;

    public static bool ShowReward(Action<bool> onReward = null)
    {
        //WorldEventSystem.Trigger("RewardWatch");
        if (Instance.IsStaging)
        {
        ...
        {
            //SignalHub.OnAdsFailedToShown.Invoke();
            return false;
        }
